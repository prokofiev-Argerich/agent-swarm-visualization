...
export async function getAgentContextStreamResponse(
  req: Request,
  agentId: string
): Promise<Response> {
  ...
  const agent = await store.getAgent({ agentId: trimmed });
  if (agent.role !== "human") {
    void runtime.wakeAgent(trimmed, "context_stream");
  }
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(controller) {
      ...
      const channel = getUpstashRealtime().channel(`agent:${trimmed}`);
